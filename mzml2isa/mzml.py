...
from .utils import etree, get_parent, pyxpath


class _CVParameter(
    collections.namedtuple(
        "_CVParameter", ["accession", "attribute", "name", "plus1", "value", "software"]
    )
):
    """A named tuple with controlled vocabulary parameter information.

    Attributes:
        attribute (bool): NO IDEA ?
...
class MzMLFile(object):
    ...
    def _extract_generic_parameters(self, meta):
        ...
        for location_name, xpath in six.iteritems(self._XPATHS_META):
            for element in self._find_xpath(xpath):
                self._extract_cv_params(element, terms[location_name], meta)
    ...
    def _extract_instrument(self, meta):
        ...
        if "Instrument" in meta:
            ...
            meta["Instrument manufacturer"] = {
                "accession": man.id,
                "name": man.name,
                "ref": self.vocabulary.meta["namespace"][0],
            }
    ...
    def _extract_scan_number(self, meta):
        ...
        self.meta["Number of scans"] = {"value": int(scan_num.attrib["count"])}
